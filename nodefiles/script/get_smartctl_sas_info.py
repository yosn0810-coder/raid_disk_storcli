...
da= os.popen("/opt/FiMo3/common/utils/runcached.py -c 300 smartctl -a "+sys.argv[1]+"|awk '/Error/ {for(i=0;i<5;i++) {getline;print}}'|awk 'NR==5{print}'")
f = da.read()
#print f
...
da= os.popen("/opt/FiMo3/common/utils/runcached.py -c 300 smartctl -a "+sys.argv[1]+" |awk '/Error/ {for(i=0;i<5;i++) {getline;print}}'|awk 'NR==4{print}'")
f = da.read()
try:
#print f
    sas_ecc_info=f.split();
