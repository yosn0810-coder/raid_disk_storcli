...
parser = argparse.ArgumentParser(
    ...
    epilog="""Example of use:
        ...
        > ./raid_arcconf_zabbix_lld.py --verbose pd -1 lld
...
def col_value(data, key, index=0):
    ...
    Args: 
        data    (str): raw output of 'arcconf'.
    ...
    for line in data.splitlines():
        columns = line.split(' : ')
        if len(columns) >= 2 and columns[0].strip() == key:
            if col_index == index:
...
if args.obj_type == 'ad':
    if args.obj_id == -1:
        for ad_num in [1]:  # Adjust range if needed
            command = f'/opt/FiMo3/common/utils/runcached.py -c 300 /usr/sbin/arcconf GETCONFIG {ad_num} AD'
            res = run(command)
            if res:
                obj_data = {'{#OBJ_ID}': str(ad_num), '{#OBJ_TYPE}': 'ad'}
                value = col_value(res, 'Controller Model')
                if value:
                    obj_data['{#OBJ_ALIAS}'] = value
                data.append(obj_data)
            else:
                logger.error(f"Failed to retrieve AD data for controller {ad_num}.")
    else:
        command = f'/opt/FiMo3/common/utils/runcached.py -c 300 /usr/sbin/arcconf GETCONFIG {args.obj_id} AD'
        res = run(command)
        if res:
...
if args.obj_type == 'ld':
    if args.obj_id == -1:
        command = '/opt/FiMo3/common/utils/runcached.py -c 300 /usr/sbin/arcconf GETCONFIG 1 LD'
        res = run(command)
        for ld_num in range(10):  # 0-9
            if last_value(res, f'Logical Device number {ld_num}'):
                obj_data = {'{#OBJ_ID}': ld_num, '{#OBJ_TYPE}': 'ld'}
                pd_diskname = col_value(res, 'Disk Name', ld_num)
                if pd_diskname:
                    obj_data['{#VIDDKPATH}'] = pd_diskname.replace('/dev/', '').strip()
                value = col_value(res, 'Logical Device name', ld_num)
                if value:
                    obj_data['{#OBJ_ALIAS}'] = value
                data.append(obj_data)
    else:
        command = f'/opt/FiMo3/common/utils/runcached.py -c 300 /usr/sbin/arcconf GETCONFIG 1 LD {args.obj_id}'
        res = run(command)
        if res:
            change_param = args.param.replace("_", " ")
            value = col_value(res, change_param)
            if value:
                if change_param == "Disk Name":
                    print(value.split('/')[2])
                else:
                    print(value)
...
if args.obj_type == 'pd':
    if args.obj_id == -1:
        command = '/opt/FiMo3/common/utils/runcached.py -c 300 /usr/sbin/arcconf GETCONFIG 1 PD'
        res = run(command)
        if res:
            for pd_num in range(10):  # 0-9
                if col_value(res, 'State', pd_num):
                    obj_data = {'{#OBJ_ID}': pd_num, '{#OBJ_TYPE}': 'pd'}
                    pd_vendor = col_value(res, 'Vendor', pd_num)
                    pd_slot = col_value(res, 'Reported Location', pd_num)
                    if pd_vendor and pd_slot:
                        obj_data['{#OBJ_ALIAS}'] = f'{pd_vendor} {pd_slot}'
                    data.append(obj_data)
    else:
        command = "/opt/FiMo3/common/utils/runcached.py -c 300 /usr/sbin/arcconf GETCONFIG 1 PD"
        res = run(command)
...
#    if not data:
#        logger.warning("No data collected; returning empty array.")
